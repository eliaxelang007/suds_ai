...
class Chatbot:
	...
	def respond(self, response_format: ResponseFormat | NotGiven = NotGiven()) -> Message:
		response = respond(
			self.messages, response_format=response_format, model=self.model)
...
class Assistant():
	...
	def __post_init__(self, create_key: object, model: AiModel, name: str, description: str, instructions: str, tools: list[AssistantTool]):
		assert create_key == type(
			self).__create_key, "Don't create this class manually; create this class through an instance of [Thread] instead!"
	...
	def __exit__(self,
			  exc_type: type[BaseException] | None, exc: BaseException | None, traceback: TracebackType | None
		):
		self.close()

	def close(self):
		openai.beta.assistants.delete(self.openai_assistant.id)
